...
def update(request, sid):
    ...
    try:
        ...
        # 处理店铺封面图片的上传
        cover_pic_file = request.FILES.get("cover_pic", None)
        if cover_pic_file:
            cover_pic = str(uuid.uuid4()) + "." + cover_pic_file.name.split('.').pop()
            with open("./static/uploads/shop/" + cover_pic, "wb+") as destination:
                for chunk in cover_pic_file.chunks():
                    destination.write(chunk)
            ob.cover_pic = cover_pic
        # 处理店铺横幅图片的上传
        banner_pic_file = request.FILES.get("banner_pic", None)
        if banner_pic_file:
            banner_pic = str(uuid.uuid4()) + "." + banner_pic_file.name.split('.').pop()
            with open("./static/uploads/shop/" + banner_pic, "wb+") as destination:
                for chunk in banner_pic_file.chunks():
                    destination.write(chunk)
            ob.banner_pic = banner_pic
